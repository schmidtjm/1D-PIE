...
######################################################################
        ...
        kappa = self.km_u/self.rhom/self.cm                                              # Thermal diffusivity 
        ...
        for i in np.arange(1,self.n_steps):
           ...
           # print('Tm',self.Tm[i])
            ...
            if self.tectonics == 'ML':
                ...
                self.ql[i] = self.km_u*((self.Tm[i] - self.Tl[i])/self.delta_s[i-1])  # Lithosphere heat flux

            else: # SL
                self.Tl[i] =self.Tm[i] - 2.9*(self.R_gas*self.Tm[i]**2/self.E)
                if (self.Dcr[i-1] == 0):
                    self.Tcr[i] = self.Ts
                    self.qs[i] = self.km_u*((self.Tl[i] - self.Ts)/self.Dl[i-1])
                    self.qs[i] = max(self.qs[i], 0.02)
                else:
                    self.Tcr[i] = suppf.calculate_conductive_two_layers(self.kcr,self.km_u,self.Qcr[i]*self.rhocr,self.Qm[i]*self.rhom,
            ...
            if self.core_cooling =='yes':
                ...
                Ra_int_b = self.rhom*self.g*self.alpha*(self.Tm[i]-self.Ts+dTc)*D**3.0/(kappa_CMB*self.etab[i]) #Baumeister et al 2023
                Racrit_int_b = 0.28*Ra_int_b**0.21  # Morschhauser et al. 2011

                # Update bottom TBL thickness
                self.delta_c[i] = (self.km_b*self.etab[i]*Racrit_int_b/(self.rhom*self.alpha*gCMB*dTc))**self.beta 
               ...
               # self.delta_c[i] = Racrit_int
                
                # CMB heat flux
                self.qc[i] = self.km_b*(self.Tc[i] - self.Tb[i])/self.delta_c[i]
           ...
           # Vertical resolution for the temperature profile
            ...
            if self.hydrous_melting =="yes":
                delta_T_sol =  43*np.minimum(self.Xm_H2O[i-1]*1e-4*(Pr*0+1),12*Pr**0.6+Pr)**0.75 
            else:
                delta_T_sol =  0.0
            ...
            if idx_meltzone.size > 0:
                valid_melt_idx = [idx for idx in idx_meltzone if Pprof[idx] < self.Pcrossover and idx < npt - 1]
                if valid_melt_idx:
                    self.Tmelt_min[i] = Tprof[valid_melt_idx[-1]]   
                    self.Tmelt_max[i] = Tprof[valid_melt_idx[0]]    
                    self.Pmelt_min[i] = Pprof[valid_melt_idx[-1]]  
                    self.Pmelt_max[i] = Pprof[valid_melt_idx[0]]   
            ...
            for idx in idx_meltzone: 
                if (Pprof[idx] < self.Pcrossover) and (idx < npt -1):  
                    ...
                    if self.partitioning_calc == 'no':
                        ...
                        S_ox = sum(oxy)  #Sum of mole oxygen
                        cat = np.multiply(mol_ox, self.n_cat)
                        S_cat = sum(cat) #sum of mole cations
                        ...
                        # molar fraction of Al on M1 lattice site
                        X_Al_M1 = CATS + JD   
                        ...
                        #Mg on M1 site 
                        X_Mg = Mg_nr*(1 - X_Cr - X_Ti - (a_cats/X_Ca)) # not used
                       ...
                       # D_Na = np.exp(((2508 + 2333*Pprof[idx]/1e9 - 138.5*(Pprof[idx]/1e9)**2)/Tprof[idx]) - 4.514 - 0.4791*Pprof[idx]/1e9 + 0.0415*(Pprof[idx]/1e9)**2) # "reference coefficient", Eq 25 along solidus, Schmidt and Noack
                        ...
                        # K
                        R0_K = (((0.974 + 0.067*X_Ca) - 0.051*X_Al_M1)+0.12)*1e-10 #1+ = 3+ + 0.12
                      ...
                      #  self.D_U_max[i]=self.D_U[i]
                        ...
                        # Ce (H2O)
                        R0_Ce = (((0.974 + 0.067*X_Ca) - 0.051*X_Al_M1))*1e-10     #3+
                        ...
                        R0_3 = (((0.974 + 0.067*X_Ca) - 0.051*X_Al_M1))*1e-10     #3+
            ...
            if self.tectonics == 'SL': 
                ...
                derivTrRl = (-self.Qm[i]*self.rhom*self.Rl[i-1]/(3.0*self.km_u) -(self.Tl[i]-self.Tcr[i]-self.Qm[i]*self.rhom*( self.Rcr[i]**2-self.Rl[i-1]**2)/(6.0*self.km_u))/(self.Rl[i-1]**2.0*(1.0/self.Rl[i-1]-  1.0/ self.Rcr[i])) )


                self.Dl[i] = (self.Dl[i-1] + (self.dt * (-self.ql[i]+(self.rhocr*self.L+self.rhocr*self.ccr*(self.Tm[i]-self.Tl[i]))*self.Dcr_prod[i]/self.dt - self.km_u*derivTrRl)/(self.rhom*self.cm*(self.Tm[i]-self.Tl[i]))) )
